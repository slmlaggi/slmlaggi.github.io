...
const navbar = document.getElementById('about-nav');

document.addEventListener('scroll', function() {
    const navbarBottom = navbar.getBoundingClientRect().bottom;
    const contentElements = document.querySelectorAll('.content'); // Adjust this selector as needed

    let isTouching = false;

    contentElements.forEach(function(element) {
        const elementTop = element.getBoundingClientRect().top;
        if (elementTop <= navbarBottom) {
            isTouching = true;
        }
    });

    if (isTouching) {
        navbar.style.backgroundColor = 'rgb(71, 64, 87)';
    } else {
        navbar.style.backgroundColor = 'rgba(25, 25, 25, 0.3)';
    }
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleBackToTopButton() {
        if (window.scrollY > firstFullscreenDiv.offsetHeight * .6) {
...
$(document).ready(function() {
    ...
    $(window).scroll(function() {
        ...
        $sections.each(function(index) {
            if (scrollPosition >= $(this).offset().top + $(this).outerHeight()) {
                currentIndex = index;
            }
        });
        updateScrollIndicator();
